...
const GoogleAssistantChat: React.FC<GoogleAssistantChatProps> = ({
  ...
  botName = "Assistant",
  userName = "Bạn",
  geminiApiKey = "AIzaSyCHIId7Q80cEF4PFDaJt6JwIG5EQuKUqvU",
}) => {
  ...
  const saveMapHistory = async (newHistory: MapHistoryItem[]) => {
    try {
      await AsyncStorage.setItem("mapHistory", JSON.stringify(newHistory));
    } catch (error) {
  ...
  const fetchRoute = async (start: Location, end: Location) => {
    ...
    try {
      const response = await axios.get(
        `http://router.project-osrm.org/route/v1/foot/${origin};${destination}?overview=full&geometries=polyline` 
      );
  ...
  const handleDirectionsRequest = async (userText: string) => {
    ...
    // Process message to extract location
    const toPattern = /đến\s+(.+)/i;
    const match = userText.match(toPattern);

    if (match) {
      endPlace = match[1].trim();
    }

    // Add to history
    addToMapHistory("Vị trí hiện tại", endPlace);

    // Create response message
    const messageId = generateId();
    const botResponse: Message = {
      id: messageId,
      text: `Đang mở Google Maps với điểm đến ${endPlace}...`,
      isUser: false,
      timestamp: new Date(),
    };
    ...
    if (endCoords) {
      ...
      openGoogleMaps(endPlace);
      speak(`Đang mở Google Maps với điểm đến ${endPlace}`);
    } else {
      ...
      setMessages((prevMessages) =>
        prevMessages.map((msg) =>
          msg.id === messageId
            ? {
                ...msg,
                text: `Không thể tìm thấy địa điểm ${endPlace}`,
              }
            : msg
        )
      );

      speak(`Không thể tìm thấy địa điểm ${endPlace}`);
    }
  };
  ...
  const openGoogleMaps = (destination: string) => {
    const url = Platform.select({
      ios: `comgooglemaps://?daddr=${encodeURIComponent(
        destination
      )}&directionsmode=walking`, // <-- Đây là chế độ đi bộ cho iOS
      android: `google.navigation:q=${encodeURIComponent(destination)}&mode=w`, // <-- Đây là chế độ đi bộ cho Android
    });
    ...
    Linking.openURL(url!).catch((err) => {
      ...
      const fallbackUrl = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(
        destination
      )}&travelmode=walking`; // <-- Đây là chế độ đi bộ cho fallback web
